...
def fresh_backup() -> dict[str, Any]:
    ...
    dump = REPORT_DIR / f"mmp_db_pre_account23_{ts}.dump"
    _run(
        [
            "docker",
            "exec",
...
def snapshot_inventory() -> dict[str, Any]:
    ...
    return {
        ...
        "queue23": _redis_llen("queue:rubika:23"),
    }
...
def host_precheck() -> dict[str, Any]:
    ...
    exists = _psql("SELECT COUNT(*) FROM accounts WHERE id=23;")
    if exists != "1":
        raise GateFailure(
            failed_stage="PRECHECK",
            failed_gate="ACCOUNT23_EXISTS",
            expected="1",
            actual=exists,
        )
    mapping = _psql(
    ...
    if mapping != "23:legacy_unclassified":
        raise GateFailure(
            failed_stage="PRECHECK",
            failed_gate="SESSION725_MAPPING_STATUS",
    ...
    if active23 != "0":
        raise GateFailure(
            failed_stage="PRECHECK",
            failed_gate="ACCOUNT23_ACTIVE_ZERO",
            expected="0",
            actual=active23,
        )
    # Account13 must remain ACTIVE Session729; global ACTIVE == 1
    a13 = _psql(
        "SELECT id||':'||session_status::text FROM channel_sessions "
        "WHERE account_id=13 ORDER BY id;"
    )
...
def promotion_committed(inner: dict[str, Any] | None) -> bool:
    """True only when Account23/725 promotion reached a committed success path."""
    if not inner:
        return False
    ...
    if inner.get("promotion_committed") is True:
        return True
    return bool(inner.get("ACCOUNT23_PROMOTION_PASS"))


def verify_account23_restored_to_prepromotion(
    before: dict[str, Any],
    *,
...
def handle_host_postcheck_failure_after_commit(
    *,
    ...
    verifier = restore_verifier or verify_account23_restored_to_prepromotion
...
async def promote_and_verify() -> dict[str, Any]:
    """Worker role: authorize again, then mutate Account23/725 only."""
    ...
    try:
        ...
        def queue_len(_aid: int) -> int:
            return int(os.environ.get("L8_QUEUE23_LEN", "0"))
        ...
        promo = await promote_proven_legacy_rubika_session(
            ...
            evidence=LegacyPromotionEvidence(
                source_phase="L8_PRODUCTION_ACCOUNT23",
                operator_note="operator-approved Account23 only",
            ),
        ...
        artifact["ACCOUNT23_PROMOTION_PASS"] = bool(inner_ok)
        artifact["ACCOUNT23_ACTIVE_SESSION_ID"] = SESSION_ID if inner_ok else None
        artifact["ACCOUNT23_CANONICAL_LOADER_PASS"] = bool(
            ver.get("canonical_ok") and ver.get("canonical_session_id") == SESSION_ID
        )
        artifact["ACCOUNT23_LEGACY_CANONICAL_MATCH"] = bool(
            artifact["LEGACY_CANONICAL_MATCH"]
        )
...
def _inventory_excluding_target(inv: str) -> str:
    """Drop session 725 from inventory for unrelated-status comparison."""
    parts = []
...
def assert_host_postconditions(
    *,
    ...
    _assert_eq(stage, "ACCOUNT23_PROMOTION_PASS", True, inner.get("ACCOUNT23_PROMOTION_PASS"))
    _assert_eq(stage, "ACCOUNT23_ACTIVE_SESSION_ID", SESSION_ID, inner.get("ACCOUNT23_ACTIVE_SESSION_ID"))
    _assert_eq(stage, "ACCOUNT23_CANONICAL_LOADER_PASS", True, inner.get("ACCOUNT23_CANONICAL_LOADER_PASS"))
    _assert_eq(
        stage,
        "ACCOUNT23_LEGACY_CANONICAL_MATCH",
        True,
        inner.get("ACCOUNT23_LEGACY_CANONICAL_MATCH"),
    )
    _assert_eq(stage, "GLOBAL_ACTIVE_SESSION_COUNT", 2, after["active_global"])
    _assert_eq(stage, "QUEUE23", 0, after["queue23"])
    _assert_eq(
        stage,
    ...
    if f"{SESSION_ID}:23:active" not in (after["session_inventory"] or ""):
        raise GateFailure(
            failed_stage=stage,
            failed_gate="SESSION725_ACTIVE_IN_INVENTORY",
...
def run_host() -> dict[str, Any]:
    ...
    for line in reversed(out.splitlines()):
        line = line.strip()
        if line.startswith("{") and (
            "ACCOUNT23_PROMOTION_PASS" in line or "FAILED_STAGE" in line
        ):
    ...
    artifact = {
        "phase": "L8_BATCH_A_ACCOUNT23_PROMOTION",
        ...
        "ACCOUNT23_ACTIVE_SESSION_ID": SESSION_ID,
...
def main() -> int:
    ...
    try:
        ...
        REPORT_DIR.mkdir(parents=True, exist_ok=True)
        (REPORT_DIR / "L8_ACCOUNT23_LAST_FAILURE.json").write_text(
